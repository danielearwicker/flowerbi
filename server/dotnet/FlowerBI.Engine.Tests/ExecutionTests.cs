...
{
    public static readonly Schema Schema = TestSchema.Schema;

    public static readonly Schema ComplicatedSchema = ComplicatedTestSchema.Schema;
    ...
    private QueryResultJson ExecuteQuery(
        QueryJson json,
        Schema schema = null,
    ...
    public void AggregationFunctions(AggregationType type)
    {
        ...
        {
            Select = ["Vendor.VendorName"],
            Aggregations = [new() { Column = "Invoice.Amount", Function = type }],
        ...
        var expectedForType = expected.Select(x =>
            (x.Item1, type == AggregationType.Min ? x.Item2 : x.Item3)
        );
    ...
    public void DoubleAggregation(bool totals)
    {
        ...
        var results = ExecuteQuery(queryJson);
        var records = results.Records.Select(x =>
            (x.Selected[0], Round(x.Aggregated[0]), x.Aggregated[1])
        );
    ...
    public void DoubleAggregationMultipleSelects()
    {
        ...
        var results = ExecuteQuery(queryJson);
        var records = results.Records.Select(x =>
            (x.Selected[0], x.Selected[1], x.Aggregated[0], x.Aggregated[1])
        );
    ...
    public void ManyToMany()
    {
        ...
        var results = ExecuteQuery(queryJson);
        var records = results.Records.Select(x =>
            (x.Selected[0], x.Selected[1], Round(x.Aggregated[0]))
        );
    ...
    public void MultipleManyToManyWithSpecifiedJoins()
    {
        ...
        var results = ExecuteQuery(queryJson);
        var records = results.Records.Select(x =>
            (x.Selected[0], x.Selected[1], x.Selected[2], x.Aggregated[0])
        );
    ...
    {
        var queryJson = new QueryJson { Select = ["Vendor.VendorName", "Tag.TagName"] };
    ...
    public void AggregationCountDistinct()
    {
        var queryJson = new QueryJson
        {
            ...
            Aggregations =
            [
                new() { Column = "Invoice.Amount", Function = AggregationType.CountDistinct },
                new()
                {
        ...
        var results = ExecuteQuery(queryJson);
        var records = results.Records.Select(x =>
            (x.Selected[0], x.Aggregated[0], x.Aggregated[1] ?? 0)
        );
    ...
    public void MultipleManyToManyWithSpecifiedJoinsAndMultipleJoinDependencies()
    {
        ...
        var results = ExecuteQuery(queryJson);
        var records = results.Records.Select(x =>
            (x.Selected[0], x.Selected[1], x.Selected[2], Round(x.Aggregated[0]))
        );
    ...
    public void CalculationsAndMultiSelect()
    {
        var queryJson = new QueryJson
        {
            ...
            OrderBy = [new OrderingJson { Type = OrderingType.Select, Index = 1 }],
        };
    ...
    public void SqlAndDapperWithListFilter()
    {
        ...
        records.Should().BeEquivalentTo([(4L, 6L, 1), (4L, 3L, 1), (9L, 3L, 1), (4L, 4L, 1)]);
    }
