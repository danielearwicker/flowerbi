...
public static class CSharp
{
    ...
    public static void FromYaml(
        ...
        FromSchema(
            ResolvedSchema.Resolve(File.ReadAllText(yamlFile)),
            csFile,
            csNamespace,
            console
        );

    static void FromSchema(
    ...
    )
    {
        ...
        foreach (var table in schema.Tables)
        {
            ...
            void WriteColumn(ResolvedColumn column, string foreignKey, string plainColumn)
            {
                ...
                var propertyDecl =
                    $"public static readonly {className}<{columnType}> {column.Name} = new {className}<{columnType}>";
