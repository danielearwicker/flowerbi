...
namespace FlowerBI
{
    public class Joins
    {
        ...
        private class TableSubset
        {
            ...
            public TableSubset(IEnumerable<LabelledTable> tables, Referrers referrers, IEnumerable<string> joinLabels)
            {
        ...
        public string ToSql(ISqlFormatter sql)
        {
            ...
            foreach (var table in reachable.Where(x => x != root))
            {
                ...
                var arrowsToAlreadyJoined = availableArrows.Where(x => joinedSoFar.Contains(x.Table)).ToList();
                ...
                output.Add($"join {table.Value.ToSql(sql)} {GetAlias(table)} on {string.Join(" and ", criteria)}");
