...
function init(db: Database) {
    ...
    const dateRows = dates
        .map(
            (x) => `
        ('${formatDate(x)}', ${x.getFullYear()}, '${startOfQuarter(
                x
            )}', '${startOfMonth(x)}')`
        )
        .join(",");
            ...
            for (const resolved of [0, 1]) {
                for (const fixedByCustomer of [0, 1]) {
                    workflows.push(`
                        (${
                            workflows.length + 1
                        }, ${resolved}, '${workflowState}', '${sourceOfError}', ${fixedByCustomer})`);
                }
            }
        }
    }
    ...
    for (var n = 0; n < 100; n++) {
        ...
        bugs.push(`
            (${
                n + 1
            }, ${workflow}, ${customer}, '${reported}', '${resolved}', '${assigned}', ${catComb}, ${coderAssigned}, ${coderResolved})`);
    }
...
export const db: Promise<Database> = (async () => {
    const sqlite3 = await sqlite3InitModule({
        print: console.log,
        printErr: console.error,
    });
    console.log("Running SQLite3 version", sqlite3.version.libVersion);
    const result = new sqlite3.oo1.DB("/mydb.sqlite3", "ct");
    init(result);
    return result;
})();
