import { type AggregationType } from "@flowerbi/client";
import {
    type BuiltFilter,
    type BuiltQuery,
    getColumnDataType,
    ...
    for (const f of filters) {
        if (f.operator && f.value) {
            ...
            if (typedValue !== undefined) {
                ...
                lines.push(
                    `${indent}{ column: ${f.table}.${f.column}, operator: "${f.operator}", value: ${literal} },`
                );
            }
        }
    }
...
function aggregationMethod(
    ...
    return `.${aggregation[0].toLocaleLowerCase()}${aggregation.substring(
        1
    )}(${filterText})`;
}
...
export function CodePreview({ query, sql }: CodePreviewProps) {
    ...
    for (const s of query.select) {
        if (s.name.trim() && s.table && s.column) {
            lines.push(
                `        ${s.name.trim()}: ${s.table}.${
                    s.column
                }${aggregationMethod(s.aggregation, s.filters)},`
            );
        }
    }
    lines.push("    }");
    ...
    if (query.ordering.length) {
        ...
        for (const o of query.ordering) {
            lines.push(
                `        { select: ${o.name} }, descending: ${o.desc} },`
            );
        }
        lines.push("    ],");
    }
